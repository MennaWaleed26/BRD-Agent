...
from .entry import arabic_entry_node, bilingual_entry_node, router_node
from .interafces_and_actors import interfaces_actors_node
from .requirements_planner import functional_req_planner_node
from .proposed_system import ProposedSystem
from .requirements_writer import Functional_Requiremenst
from .timeline import Timeline
from .merger import MergeProposal


def create_brd_graph():

    proposed_system_nodes =ProposedSystem()
    proposed_system_billingual = proposed_system_nodes.proposed_system_billingual
    proposed_system_ar = proposed_system_nodes.proposed_system_ar


    timeline_nodes = Timeline()
    timeline_billingual = timeline_nodes.timeline_billingual
    timeline_ar = timeline_nodes.timeline_ar

    validate_timeline_bi= timeline_nodes.validate_timeline_bi
    validate_timeline_ar = timeline_nodes.validate_timeline_ar

    timeline_fallback_bi_node = timeline_nodes.timeline_fallback_bi_node
    timeline_fallback_ar_node = timeline_nodes.timeline_fallback_ar_node

    finish_timeline_bi_node = timeline_nodes.finish_timeline_bi_node
    finish_timeline_ar_node = timeline_nodes.finish_timeline_ar_node

    router_after_timeline_validation = timeline_nodes.router_after_timeline_validation



    functional_nodes = Functional_Requiremenst()
    functional_requirements_ar = functional_nodes.functional_requirements_ar
    functional_requirements_bill = functional_nodes.functional_requirements_bill


    merger = MergeProposal()
    Final_BRD_billingual = merger.Final_BRD_billingual
    Final_BRD_ar = merger.Final_BRD_ar


    
    workflow=StateGraph(GraphState)
    
    workflow.add_node("interfaces_actors_node", interfaces_actors_node)
    ...
    workflow.add_node("functional_requirements_bill",functional_requirements_bill)
    ...
    workflow.add_node("functional_requirements_ar", functional_requirements_ar)
    ...
    workflow.add_edge(START, "interfaces_actors_node")
    workflow.add_edge("interfaces_actors_node", "functional_req_planner")
    ...
    workflow.add_edge("bilingual_entry_point","functional_requirements_bill")
    workflow.add_edge([
        "proposed_system_bi",
        "finish_timeline_bi_node",
        "functional_requirements_bill"
    ],"final_brd_bi")
    ...
    workflow.add_edge("arabic_entry_point","functional_requirements_ar")
