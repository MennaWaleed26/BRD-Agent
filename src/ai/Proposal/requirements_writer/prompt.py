...
functional_requirements_group_bill_template = PromptTemplate(
...
You are writing the detailed functional scope section of a commercial software
proposal for a non-technical Arabic-speaking client.

The section is client-facing. It must explain the system in practical business
language without becoming a technical software specification.
...
{functional_requirements_plan}

Source hierarchy:

1. Modules and Features defines the exact interfaces, modules, and features.
2. Project Details supplies confirmed workflow details and business rules.
3. Interfaces and Users defines the users associated with each interface.
4. Do not create functionality beyond these sources.

Writing hierarchy:

- Interface introduction:
  Explain the overall purpose of the interface and its main users.

- Module overview:
  Explain the broad functional area covered by the module and why it exists.
  Do not describe individual feature workflows here.

- Feature purpose:
  Explain the distinct result delivered by the feature.
  It must add meaningful information beyond the module overview.

- Feature user journey:
  Explain what the user does and what outcome the system produces.

- Feature business rules:
  Include only confirmed constraints or conditions governing that feature.

- Feature business value:
  State the direct operational value of that specific feature.

Instructions:

- Generate one group for every canonical interface in the same order.
- Copy every group_key, module_key, and feature_key exactly.
- Represent every canonical module and feature exactly once.
- Do not create, omit, merge, split, or move modules or features.
- Use professional Modern Standard Arabic for a non-technical client.
- Keep every module overview concise: one or two sentences.
- Do not place feature workflows, detailed rules, or feature benefits in the
  module overview.
- Every feature must add meaningful information beyond its parent module.
- Do not merely restate the feature title.

Weak example:
Title: عرض الحضور اليومي
Purpose: يتيح عرض الحضور اليومي.

Better example:
Title: عرض الحضور اليومي
Purpose: يمنح الموارد البشرية رؤية موحدة لسجلات حضور الموظفين خلال اليوم،
بما يساعد على اكتشاف الحالات التي تحتاج متابعة.

- Generate user_journey_ar only when the feature has a meaningful sequence.
- Use two to five concise business-oriented steps.
- Describe user actions and system outcomes.
- Do not mention databases, APIs, frameworks, code, tables, storage, endpoints,
  UUIDs, infrastructure, or implementation libraries.
- Do not invent screen counts, field names, limits, channels, or workflow stages.
- Include only confirmed business rules.
- When a threshold is not specified, refer to it generically.

Correct:
"يصدر تنبيه عند تجاوز الحد المعتمد للغياب."

Wrong:
"يصدر تنبيه بعد ثلاثة أيام غياب."

- Add no more than two direct business benefits per feature.
- Do not repeat the feature purpose as a business benefit.
- Do not mention MVP scope, phases, pricing, or duration.
- Preserve the client's domain terminology.
"""

functional_requirements_group_ar_template = PromptTemplate(
    template=FUNCTIONAL_REQUIREMENTS_GROUP_AR_TEMPLATE,
    input_variables=["project_title","interfaces_and_users","project_details","functional_requirements_plan"]
)
