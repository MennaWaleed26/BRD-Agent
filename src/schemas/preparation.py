...
class PreparationOutput(BaseModel):
    business_need: str = Field(
        ...,
        description=(
            "A concise explanation of the main operational or business problem "
    ...
    business_objectives: list[str] = Field(
        ...
        description=(
            "The main business outcomes explicitly stated or clearly supported "
            "by the project input."
        ),
    )
    ...
    main_workflows: list[str] = Field(
        default_factory=list,
        description=(
            "The main end-to-end business workflows, expressed at a high level. "
            "Do not list individual features."
        ),
    )

    cross_cutting_business_rules: list[str] = Field(
        default_factory=list,
        description=(
            "Important rules, validations, approval conditions, permissions, "
            "or restrictions that affect multiple modules or workflows."
        ),
    )

    key_scope_expectations: list[str] = Field(
        default_factory=list,
        description=(
            "Important scope expectations that later nodes must not overlook, "
            "such as reporting, notifications, auditability, anti-fraud controls, "
            "exports, or multi-portal operation."
        ),
    )
    ...
    domain_terms: list[str] = Field(
        default_factory=list,
        description=(
            "Important client-specific or industry-specific terms that should "
            "remain consistent throughout the proposal."
            "make them in the same input language as translating willmake them loose their meaning "
        ),
    )
    ...
    ambiguities: list[str] = Field(
        default_factory=list,
        description=(
            "Material contradictions, missing decisions, or unclear points that "
            "could affect platform reconciliation, scope, or delivery planning."
        ),
    )
