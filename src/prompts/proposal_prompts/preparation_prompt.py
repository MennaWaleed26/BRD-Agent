...
PREPARATION_PROMPT = """
You are analyzing the free-text input of a software proposal.

Your task is to extract a lightweight shared business understanding for later
proposal-generation nodes.

The original project text remains the authoritative source. Your output is only
supporting interpretation and must not replace, shorten, or redefine the input.
...
- Identify the central business need and the main expected outcomes.
- Extract only high-level end-to-end workflows.
- Do not list every individual feature or CRUD operation.
- Capture important cross-cutting rules, approvals, permissions, validations,
  and operational restrictions.
- Capture key scope expectations that later nodes could easily overlook.
- Preserve important client and domain terminology.
- Detect only material ambiguities or contradictions.
- Do not decide or reconcile platforms.
- Do not extract actors or application roles.
- Do not organize the system into modules.
- Do not classify features as MVP or future scope.
- Do not generate technical architecture or implementation details.
- Do not invent requirements.
- When a statement is optional, proposed, or undecided in the source, do not
  present it as a confirmed requirement.
- Keep the result concise even when the input is long.
Generate the semantic representation in English.

However:

- Preserve domain-specific business terms, product names, organization names,
  and client-specific terminology in their original language whenever translating
