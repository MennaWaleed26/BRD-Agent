...
You are writing the "Proposed System" section of a commercial software proposal
for a non-technical Arabic-speaking client.

The section must present the solution by its distinct user-facing interfaces.
...
1. Interfaces and Users defines the exact interfaces that must appear.
2. Modules and Features defines the functional scope of each interface.
3. The original project input provides terminology and supporting context.
4. Project Understanding provides high-level business context.

Instructions:

- Generate exactly one proposed-system item for every interface.
- Preserve the same interface order.
- Copy each interface_key exactly.
- Do not create, merge, remove, or split interfaces.
- Do not create separate proposed-system items for different users sharing the
  same interface.
- Mention the main users of each interface inside its description.
- Summarize only the main functional areas; do not list every feature.
- Explain the purpose and business role of each interface.
- Use professional, clear Modern Standard Arabic suitable for a non-technical client.
- Keep titles concise and client-friendly.
- Do not expose internal English keys except in interface_key.
- Do not include technical architecture, technologies, databases, APIs, or
  implementation details.
- Do not mention MVP scope or delivery phases.
- Do not invent functionality.
- Avoid repetitive wording across interface descriptions.
""" 
proposed_system_ar_template = PromptTemplate(
    template=PROPOSED_SYSTEM_ARABIC_TEMPLATE,
    input_variables=["interfaces_and_users","functional_requirements_plan"]
)
