...
The section is client-facing. It must explain what each module provides and how
its main functions operate, without becoming a technical software specification.
...
1. Modules and Features defines the exact interfaces, modules, and features that
   must be represented.
2. Interfaces and Users defines the users associated with every interface.
3. The original project details provide workflow stages, business rules,
   terminology, and supporting explanation.
4. Project Understanding provides shared business context.

Instructions:

- Generate one group for every interface, using the same order.
- Copy each interface key into group_key exactly.
- Generate one module item for every canonical module under that interface.
- Copy every module_key and feature_key exactly.
- Represent every canonical feature exactly once.
- Do not create, omit, merge, or move modules and features.
- Use clear Modern Standard Arabic suitable for a non-technical client.
- Explain modules in business and operational language.
- Describe how a module works when the source provides a meaningful workflow.
- Keep workflow steps sequential and client understandable.
- Include only confirmed business rules from the source.
- Explain business value without exaggerated marketing claims.
- Do not include technical architecture, database tables, APIs, frameworks,
  infrastructure, or code-level behavior.
- Do not write formal developer requirements, user stories, preconditions,
  acceptance criteria, or test cases.
- Do not mention MVP scope, phases, pricing, or delivery duration.
- Do not invent features, rules, notifications, reports, or integrations.
- Avoid repeating the same explanation across module overview, workflow, and
  feature descriptions.
- Use the client's domain terminology consistently.
- If a module has no meaningful sequential workflow, return an empty workflow_ar.
- If no confirmed business rule exists, return an empty business_rules_ar.
"""

functional_requirements_group_ar_template = PromptTemplate(
    template=FUNCTIONAL_REQUIREMENTS_GROUP_AR_TEMPLATE,
    input_variables=["project_title", "project_understanding","interfaces_and_users", "functional_requirements_plan"]
)
#  """
